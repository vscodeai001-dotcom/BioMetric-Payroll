...
doc_candidates = [
    ROOT / 'docs/1100/1100-I_EMPLOYEE_SQL_DEPENDENCY_AUDIT.md',
    ROOT / 'docs_1100/1100-I_EMPLOYEE_SQL_DEPENDENCY_AUDIT.md',
]
...
if doc_path:
    ...
    # Historical 1100 documentation is not shipped in this staged package.
    # Use the current code boundary as the regression evidence instead.
    calc_files = list((ROOT/'Web').rglob('*.cs'))
    calc_text = '\n'.join(p.read_text(encoding='utf-8', errors='ignore') for p in calc_files)
    checks.append(('SQL calculation/mutation boundaries remain present in current source', 'calculation' in calc_text.lower() and 'mutation' in calc_text.lower()))
